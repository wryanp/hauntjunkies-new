/**
 * Test the ticket confirmation email with McCloud Manor logo
 * Run with: node test-logo-email.js YOUR_EMAIL@example.com
 */
...
const TEST_EMAIL = process.argv[2] || 'vilonte1@gmail.com';
...
<head>
	...
	<style>
		...
		.header-bg {
			...
			border-bottom: 4px solid #a41214;
			text-align: center;
		}
	...
	<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color: #f4f4f4;">
		<tr>
			<td align="center" style="padding: 40px 20px;">
				<table width="600" cellpadding="0" cellspacing="0" class="email-container" style="max-width: 600px; width: 100%; background-color: #fffffe; box-shadow: 0 4px 20px rgba(0,0,0,0.1); border-radius: 8px; overflow: hidden;">

					<!-- Header -->
					<tr>
						<td align="center" class="header-bg" style="padding: 50px 40px; background-color: #000001; border-bottom: 4px solid #a41214; vertical-align: middle;">
							<!-- McCloud Manor Logo -->
						...
						<td style="padding: 0 40px 30px 40px;">
							<table width="100%" cellpadding="12" cellspacing="0" style="background-color: #f9f9f9; border-radius: 8px; border: 1px solid #e0e0e0;">
								...
								<tr>
									...
									<td style="padding: 8px; color: #666; border-bottom: 1px solid #e0e0e0;">Logo Test</td>
								</tr>
								<tr>
...
async function sendTest() {
	try {
		console.log('📧 Sending test email with McCloud Manor logo...');

		const { data, error } = await resend.emails.send({
			from: 'Haunt Junkies <noreply@hauntjunkies.com>',
			to: [TEST_EMAIL],
			subject: '🎃 Your McCloud Manor Tickets (Logo Test)',
			html: htmlContent
		});
		...
		console.log('   ✓ McCloud Manor logo appears in the header');
		console.log('   ✓ Logo is centered');
		console.log('   ✓ Logo is properly sized (~300px wide)');
		console.log('   ✓ The text "MCCLOUD MANOR" is removed');
