...
-- Ticket dates table (for managing available dates and capacity)
...
CREATE INDEX idx_ticket_dates_date ON ticket_dates(date);
CREATE INDEX idx_ticket_dates_available ON ticket_dates(is_available);
CREATE INDEX idx_quotes_active ON quotes(is_active);
CREATE INDEX idx_quotes_order ON quotes(display_order);
...
-- Ticket dates policies
CREATE POLICY "Public can read available ticket dates" ON ticket_dates
  FOR SELECT USING (is_available = true);

CREATE POLICY "Authenticated users can manage ticket dates" ON ticket_dates
  FOR ALL USING (auth.role() = 'authenticated');
