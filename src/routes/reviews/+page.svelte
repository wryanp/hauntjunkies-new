							...
							<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
								{#each groupedReviews[state][year] as review}
									<a
										...
										class="group bg-gray-800/50 rounded-lg overflow-hidden hover:bg-gray-800 transition-all duration-300 relative
										       {hasGoldenGhostAwards(review)
										         ? 'border-2 border-yellow-500/70 hover:border-yellow-400 shadow-lg shadow-yellow-500/20 hover:shadow-yellow-500/40'
										         : 'border border-gray-700 hover:border-haunt-orange'}"
									>
													...
													<div class="flex items-center gap-0.5">
														{#each Array(5) as _, i}
															<img
																src="/ghost.png"
																alt="Rating ghost"
																class="w-7 h-7 object-contain transition-all {i < Math.round(review.rating_overall) ? 'opacity-100 brightness-110' : 'opacity-20 grayscale'}"
