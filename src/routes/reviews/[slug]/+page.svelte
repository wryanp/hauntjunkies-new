...
<SEO
	...
	image={data.review.review_image || data.review.cover_image_url || '/og.jpg'}
	type="article"
	article={{
					...
					{#each data.comments as comment}
						<div class="bg-gray-900/50 rounded-lg p-4 border border-gray-700">
							...
							<div class="mt-3 flex items-center gap-2">
								<form method="POST" action="?/likeComment" use:enhance={() => {
									return async ({ update }) => {
										await update();
									};
								}}>
									...
									<button
										type="submit"
										class="group flex items-center gap-1.5 text-sm text-gray-400 hover:text-haunt-orange transition-colors"
										aria-label={data.likedCommentIds.includes(comment.id) ? 'Unlike comment' : 'Like comment'}
									>
										{#if data.likedCommentIds.includes(comment.id)}
											<!-- Filled heart (liked) -->
										...
										{:else}
											...
											<svg class="w-5 h-5 group-hover:text-haunt-orange transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
